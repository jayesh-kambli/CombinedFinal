...
";
    ...
    ";
        ...
        foreach ($result as $row) {
            ...
            // $sub_array[] = $row["start_id"]; // Add start_id here
            $sub_array[] = "<buttonA type='button' name='report_button' start-date=".$row["start_id"]." data-student_id='" . $row["student_name"] . " (" . $row["student_id"] . ")' class='btn btn-info btn-sm report_button' data-attendance='" . json_encode($row["attendance_data"]) . "' data-bs-toggle='modal' data-bs-target='#ModalForReports'>Report</buttonA>";
            $data[] = $sub_array;
        }
