...
AND attendance_date_time >= '".$_GET["from_date"]."' 
AND attendance_date_time <= '".$_GET["to_date"]."'
";
...
{
 $status = '';
 if($row["attendance_data"] == "Present")
 {
  $total_present++;
  $status = '<span class="badge badge-success">Present</span>';
 }

 if($row["attendance_data"] == "Absent")
 {
  $total_absent++;
 ...
 $output .= '
  <tr>
   <td>'.$row["attendance_date_time"].'</td>
   <td>'.$status.'</td>
  </tr>
 ';
